	...
	if(_transportMode != RTP_OVER_MULTICAST)
    {
        for(int chn=0; chn<MAX_MEDIA_CHANNEL; chn++)
        {
            if(_rtpfd[chn] > 0)
            {
               ...
               if(RTP_DEBUG)
                   cout<<"rtp close "<<_rtpfd[chn]<<endl;
            }
            ...
            if(_rtcpfd[chn] > 0)
            {
                ...
                if(RTP_DEBUG)
                    cout<<"rtcp close "<<_rtcpfd[chn]<<endl;
            }
        }
    }
}
...
bool RtpConnection::setupRtpOverUdp(MediaChannelId channelId, uint16_t rtpPort, uint16_t rtcpPort)
{
    ...
    //2018.8.6
    if(RTP_DEBUG) {
        cout << "rtp port:" << _mediaChannelInfo[channelId].rtpPort << endl;
        cout << "rtcp port" << _mediaChannelInfo[channelId].rtcpPort << endl;
    }
    return true;
}
